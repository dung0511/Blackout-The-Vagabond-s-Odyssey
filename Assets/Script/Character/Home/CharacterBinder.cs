...
public class CharacterBinder : MonoBehaviour
{
    ...
    private void Start()
    {
        GameObject lastSessionCharacter = null;

        if (GameManager.Instance.playerCharacter?.home?.name != null)
        {
            lastSessionCharacter = GameObject.Find(GameManager.Instance.playerCharacter.home.name);
        }

        BindCharacter(lastSessionCharacter ?? selectedCharacter);
    }
    ...
    public void BindCharacter(GameObject newCharacter)
    {
        ...
        interactComponent = selectedCharacter.GetComponentInChildren<SelectableCharacter>().gameObject;
